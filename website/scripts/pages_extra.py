...
def build_threads_page(books, output_dir, env, threads, tindex):
    ...
    for bi, b in enumerate(books):
        ...
        for th in threads:
            ...
            if n:
                ti = tids.index(th['id'])
                w = 1 + round(2.5 * n / maxn, 1)
                parts.append('<line class="edge" data-book="' + b['id'] + '" data-thread="' + th['id'] + '" data-n="' + str(n) + '" x1="250" y1="' + str(y + 20) + '" x2="750" y2="' + str(30 + ti * RH + 20) + '" stroke="var(--brand)" stroke-width="' + str(w) + '" opacity=".5"><title>' + htmllib.escape(b['title']) + ' × ' + htmllib.escape(th['label']) + ': ' + str(n) + ' sheets</title></line>')
    for bi, b in enumerate(books):
        y = 30 + bi * RH
        col = CAT_HEX.get(b['category'], '#18E299')
        label = htmllib.escape(b['title'][:26])
        parts.append('<g class="bnode" data-book="' + b['id'] + '" tabindex="0" role="button" aria-label="' + htmllib.escape(b['title']) + '"><rect x="20" y="' + str(y) + '" width="230" height="40" rx="10" fill="var(--card)" stroke="' + col + '" stroke-width="2"/><text x="32" y="' + str(y + 25) + '" font-size="13" font-weight="700" fill="var(--ink)">' + label + '</text><text class="gpct" data-gprog-b="' + b['id'] + '" x="238" y="' + str(y + 25) + '" font-size="10" text-anchor="end" fill="var(--mut)"></text></g>')
    for ti, th in enumerate(threads):
        y = 30 + ti * RH
        n = len(tindex[th['id']])
        parts.append('<g class="tnode" data-thread="' + th['id'] + '" tabindex="0" role="button" aria-label="' + htmllib.escape(th['label']) + '"><rect x="750" y="' + str(y) + '" width="230" height="40" rx="10" fill="var(--card)" stroke="#0e7490" stroke-width="2"/><text x="762" y="' + str(y + 25) + '" font-size="13" font-weight="700" fill="var(--ink)">' + htmllib.escape(th['label']) + ' (' + str(n) + ')</text></g>')
    svg = '<svg id="graph" width="100%" viewBox="0 0 ' + str(W) + ' ' + str(H) + '" style="max-height:70vh;border:1px solid var(--line);border-radius:14px;background:var(--card)" role="img" aria-label="Book to thread coverage graph">' + ''.join(parts) + '</svg>'
    ...
    for th in threads:
        ...
        icon = THREAD_ICON.get(th['id'], '◉')
        t_ms = sum(next((b.get('time_minutes',30) for b in books if b['id']==bid),30) for bid in bset)
        tools = THREAD_TOOLS.get(th['id'], [])[:3]
        ticons = ''.join('<span class="mini-icon" title="' + htmllib.escape(t) + '">' + htmllib.escape(t[:1]) + '</span>' for t in tools)
        cards.append('<a class="thread-card" href="#' + th['id'] + '" style="--cat:#0e7490" data-thread="' + th['id'] + '"><span class="thread-ico" aria-hidden="true">' + icon + '</span><span class="thread-num">' + str(n) + ' sheets · ' + str(nb) + ' books · ~' + str(t_ms) + ' min</span><b>' + htmllib.escape(th['label']) + '</b><span>' + htmllib.escape(th['desc']) + '</span><span class="tbar" aria-hidden="true"><b style="width:' + str(round(100 * n / maxn)) + '%"></b></span><div class="thread-tools">' + ticons + '<span class="thread-meta"><span class="tprog" data-tprog="' + th['id'] + '"></span>jump ↓</span></div></a>')
