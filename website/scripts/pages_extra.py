...
def build_threads_page(books, output_dir, env, threads, tindex):
    ...
    page = ('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">'
            ...
            "document.querySelectorAll('.thread-card').forEach(c=>c.addEventListener('click',e=>{const id=c.dataset.thread;const sec=document.getElementById(id);if(!sec)return;e.preventDefault();sec.scrollIntoView({behavior:'smooth'});sec.querySelector('.sechead').setAttribute('aria-expanded','true');sec.querySelector('.secbody').hidden=false;}));" +
