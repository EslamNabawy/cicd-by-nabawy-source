...
def build_threads_page(books, output_dir, env, threads, tindex):
    ...
    page = ('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">'
            ...
            "document.querySelectorAll('.tquiz').forEach(b=>b.onclick=()=>{const id=b.dataset.tquiz;const box=document.querySelector('#tquiz-'+id);box.hidden=!box.hidden;if(box.hidden)return;const chips=[...document.querySelectorAll('#'+id+' .chip')].map(a=>a.textContent.trim()).filter(Boolean);let qs=[...new Set(chips)].sort(()=>Math.random()-.5).slice(0,5);if(qs.length<3)qs=chips.slice(0,5);let qi=0,sc=0,st=0,miss=[];function show(){if(qi>=qs.length){box.innerHTML='<p><b>Done — '+sc+'/'+qs.length+'</b> streak '+st+(miss.length?' Missed: '+miss.join(', '):'')+' <button class=\"btn\" onclick=\"this.closest(\\'.tquizbox\\').hidden=true\">Close</button> '+(miss.length?'<button class=\"btn\" onclick=\"this.closest(\\'.tquizbox\\').__retry()\">Retry missed</button>':'')+'</p>';box.__retry=()=>{qs=[...miss];qi=0;sc=0;st=0;miss=[];show();};if(sc===qs.length)try{localStorage.setItem('cicdlib:threadbadge:'+id,'1')}catch(e){};return;}const cur=qs[qi];const opts=[cur];while(opts.length<4){const r=chips[Math.floor(Math.random()*chips.length)];if(!opts.includes(r))opts.push(r);}opts.sort(()=>Math.random()-.5);box.innerHTML='<div class=\"qprog2\" style=\"height:4px;background:var(--bg2);border-radius:99px;margin:6px 0;overflow:hidden\"><b style=\"display:block;height:100%;background:var(--brand);width:'+Math.round(100*qi/qs.length)+'%\"></b></div><p><b>Q'+(qi+1)+'/'+qs.length+' — '+cur+'</b> <small>· streak '+st+'</small></p><div style=\"display:grid;gap:6px\">'+opts.map(o=>'<button class=\"btn qo\">'+o+'</button>').join('')+'</div><div class=\"qexplain\" hidden></div>';const ex=box.querySelector('.qexplain');box.querySelectorAll('.qo').forEach(btn=>btn.onclick=()=>{const ok=btn.textContent===cur;const all=box.querySelectorAll('.qo');all.forEach(x=>{x.disabled=true;if(x.textContent===cur)x.classList.add('correct');else if(x===btn&&!ok)x.classList.add('wrong');});if(ok){sc++;st++;}else{st=0;miss.push(cur);}ex.textContent=ok?'✓ — keep going':'✗ correct: '+cur;ex.hidden=false;setTimeout(()=>{qi++;show();}, ok?800:1500);});}show();});" +
            ...
            "function isolateThread(id){_sel=id;paint(null,id);const card=document.querySelector('.thread-card[data-thread=\"'+id+'\"]');const nm=card?card.querySelector('b').textContent:id;info('<b>'+nm+'</b> isolated — <a href=\"#'+id+'\">jump to section</a> · <a href=\"#\" class=\"greset\">reset</a>');const r=_gi.querySelector('.greset');if(r)r.onclick=e=>{e.preventDefault();resetGraph();};}" +
            "function isolateBook(id){_sel='b:'+id;paint(id,null);info('<b>'+id+'</b> isolated — <a href=\"../read/'+id+'.html\">open book</a> · <a href=\"#\" class=\"greset\">reset</a>');const r=_gi.querySelector('.greset');if(r)r.onclick=e=>{e.preventDefault();resetGraph();};}" +
...
'''
    page = ('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">'
            ...
            '</div><footer><div class="wrap"><span>CICD BY Nabawy</span><span><a href="../index.html">Home</a> · <a href="../glossary.html">Study Deck</a> · <a href="../threads/">Threads</a></span></div></footer><style>.qprog2{height:6px;border-radius:99px;background:var(--bg2);margin:8px 0;overflow:hidden}.qprog2 b{display:block;height:100%;background:var(--brand)}.qexplain{margin-top:8px;padding:8px 10px;border-radius:8px;background:var(--bg2);border:1px solid var(--line-soft);font-size:12px;color:var(--mut)}.qbox .correct{border-color:var(--brand) !important;background:color-mix(in srgb,var(--brand) 10%,transparent) !important}.qbox .wrong{border-color:var(--red) !important;background:color-mix(in srgb,var(--red) 8%,transparent) !important}</style>'
            '<script>'
