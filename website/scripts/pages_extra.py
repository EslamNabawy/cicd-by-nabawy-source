...
def build_threads_page(books, output_dir, env, threads, tindex):
    ...
    for thi, th in enumerate(threads):
        ...
        first = (thi == 0)
        ticons2 = ''.join('<span class="toolpill" title="' + htmllib.escape(t) + '">' + htmllib.escape(t) + '</span>' for t in tools)
        secs.append('<section id="' + th['id'] + '" style="margin:26px 0;scroll-margin-top:70px"><h2 class="sec"><button class="sechead" aria-expanded="' + ('true' if first else 'false') + '">' + htmllib.escape(th['label']) + ' <span class="map-pill"><b>' + str(len(tindex[th['id']])) + '</b> sheets</span> <span class="chev">▾</span></button></h2><div class="secbody"' + ('' if first else ' hidden') + '><p class="sub">' + htmllib.escape(th['desc']) + ' Guided path: foundations first, depth later — ' + str(step) + ' stops · ~' + str(t_ms) + ' min · <span class="tprog" data-tprog="' + th['id'] + '"></span></p>' + ('<div class="mark m-key"><span class="tag">WHY IT MATTERS</span>' + htmllib.escape(why) + '</div>' if why else '') + ('<div class="m-exam"><span class="tag">🎓 EXAM</span>' + htmllib.escape(exam) + '</div>' if exam else '') + ('<div class="mark m-tip"><span class="tag">REAL STORY</span>' + htmllib.escape(story) + '</div>' if story else '') + ('<div style="display:flex;gap:6px;flex-wrap:wrap;margin:8px 0">' + ticons2 + '</div>' if ticons2 else '') + ('<ul class="checklist">' + cks + '</ul>' if cks else '') + '<p><button class="btn tquiz" data-tquiz="' + th['id'] + '">Quiz this thread (5)</button></p><div class="tquizbox" id="tquiz-' + th['id'] + '" hidden></div>' + ''.join(glist) + xlinks + '<p><a href="#graph">↑ graph</a> · <a href="#thread-filter">↑ filter</a></p></div></section>')
