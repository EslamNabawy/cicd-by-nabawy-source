...
#fsbar{display:none;position:fixed;left:50%;transform:translateX(-50%);bottom:max(14px,env(safe-area-inset-bottom));z-index:80;background:var(--card);border:1px solid var(--line);border-radius:9999px;padding:6px 8px;gap:4px;box-shadow:var(--shadow-lift);align-items:center;max-width:96vw}
...
#fsbar .btn{min-height:44px;padding:6px 12px}
#fs-prog{font-family:var(--mono);font-size:12px;color:var(--mut);padding:0 8px;white-space:nowrap}
.drawerx{position:absolute;top:8px;right:8px;width:44px;height:44px;display:inline-flex;align-items:center;justify-content:center;background:transparent;border:1px solid var(--line);border-radius:10px;color:var(--mut);font-size:20px;cursor:pointer;z-index:2}
.drawerx:hover{color:var(--ink);border-color:var(--brand)}
.readbody li.step{list-style:none;display:flex;gap:10px;align-items:flex-start;margin:8px 0}
.readbody li.step input[type=checkbox]{width:22px;height:22px;flex:none;margin-top:2px;accent-color:var(--brand)}
...
@media(max-width:700px){.readcontext{padding:8px 14px;font-size:12px}.readcontext .wrap{gap:7px}.readback{order:-1}.readcontext .here{flex-basis:100%;overflow-wrap:anywhere}.readcontext .pathpos,.readcontext .prereq{max-width:100%;white-space:normal}.upnext{align-items:flex-start;flex-wrap:wrap;padding:12px 14px}.upnext a{overflow-wrap:anywhere}}
...
function lockScroll(){const any=$('#drawer').classList.contains('open')||document.querySelector('aside.toc').classList.contains('open');document.body.classList.toggle('lock',any);document.querySelector('aside.toc').setAttribute('aria-modal',document.querySelector('aside.toc').classList.contains('open')?'true':'false');document.querySelector('#drawer .panel').setAttribute('aria-modal',$('#drawer').classList.contains('open')?'true':'false');}
...
const _tocx=$('#tocx');if(_tocx)_tocx.onclick=()=>{document.querySelector('aside.toc').classList.remove('open');lockScroll();};
...
const _ft=$('#fs-toc');if(_ft)_ft.onclick=()=>{document.querySelector('aside.toc').classList.toggle('open');lockScroll();};
...
{contextbar}
<div class="rlayout">
<aside class="toc" role="dialog" aria-modal="false" aria-label="Table of contents"><button class="drawerx" id="tocx" aria-label="Close contents">×</button><h4>CONTENTS · {len(pages)} PAGES · {len(toc)} SECTIONS</h4>{toc_html}
</aside>
