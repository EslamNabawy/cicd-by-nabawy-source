...
.readbody code,.readbody .mono{overflow-wrap:anywhere}.readbody table{max-width:100%}.readbody td,.readbody th{overflow-wrap:anywhere}.readbody .step .mono{overflow-wrap:break-word}
...
header.top #themebtn{width:28px;min-width:28px;min-height:44px;padding:0}header.top .wrap{padding-left:max(8px,env(safe-area-inset-left));padding-right:max(8px,env(safe-area-inset-right))}
}
...
.chiprow{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}@media(max-width:600px){.chiprow a,.chiprow .chip{padding:8px 14px;min-height:44px;display:inline-flex;align-items:center}.sechead{width:44px;height:44px}.morebtn{min-height:44px}.tquiz{min-height:44px}.qback .btn{min-height:44px}}
...
.hm-band{display:flex;justify-content:center;gap:5px;margin-bottom:18px}@media(max-width:600px){.hm-band{margin-bottom:12px}.hm-band i{width:7px}}
...
.fcard-inner{height:auto;min-height:150px;display:block;transform:none !important}
.fcard-front,.fcard-back{position:relative;inset:auto;grid-area:auto;transform:none !important;height:auto;min-height:150px;backface-visibility:visible;-webkit-backface-visibility:visible}
.fcard.flipped .fcard-inner{transform:none}
.fcard-back{transform:none;display:none}
.fcard.flipped .fcard-front{display:none}
.fcard.flipped .fcard-back{display:flex;max-height:none}
.fcard .fbtn{min-height:44px;font-size:14px}
.fcard-back p{font-size:14px;line-height:1.55}
.gtbar{top:52px}
.az{padding:6px 2px}
...
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><title>Hands-on Labs — CICD BY Nabawy</title>{seo_tags("Hands-on Labs — CICD BY Nabawy", "Direct HTML manuals for every CI/CD lab.", f"{SITE_URL}/labs/")}<style>{BASE_CSS}.lab-index{{max-width:920px;margin:0 auto;padding:54px 24px 90px}}.lab-index h1{{font-size:42px;margin:12px 0}}.lab-index p{{color:var(--mut);font-size:17px;max-width:680px;line-height:1.6}}.lab-index ul{{list-style:none;padding:0;margin:34px 0;display:grid;gap:10px}}.lab-index li{{display:flex;justify-content:space-between;align-items:center;gap:18px;padding:18px 20px;background:var(--card);border:1px solid var(--line-soft);border-radius:12px}}.lab-index li span{{color:var(--mut);font-size:13px}}@media(max-width:600px){{.lab-index{{padding:34px 14px}}.lab-index h1{{font-size:32px}}.lab-index li{{display:block}}.lab-index li span{{display:block;margin-top:6px}}}}</style></head><body><header class="top"><div class="wrap"><a class="logo" href="../index.html" style="color:inherit">CICD<span> BY Nabawy</span></a><a class="btn" href="../index.html">Home</a></div></header><main class="lab-index"><span class="eyebrow">HANDS-ON LABS</span><h1>Practice the pipeline.</h1><p>One hands-on handbook from the first green check through Jenkins recovery and DR drills. Each lab includes setup, execution, verification, failure scenarios, and cleanup.</p><ul>{lab_links}</ul></main></body></html>'''
    ...
    for t, d, r in rows:
        ...
        qterms.append((t, d.strip(), rl, slug))
    ...
    _qjson = json.dumps([[html.escape(t), html.escape(d), html.escape(rl), s] for t, d, rl, s in qterms], ensure_ascii=False)
...
.fcard{{perspective:900px;cursor:pointer}}
.fcard-inner{{position:relative;width:100%;display:grid;transition:transform .45s;transform-style:preserve-3d;min-height:150px}}
.fcard.flipped .fcard-inner{{transform:rotateY(180deg)}}
.fcard-front,.fcard-back{{grid-area:1/1;display:flex;flex-direction:column;gap:6px;padding:16px;border:1px solid var(--line-soft);border-radius:14px;background:var(--card);backface-visibility:hidden;-webkit-backface-visibility:hidden}}
.fcard-back{{transform:rotateY(180deg);overflow:auto;max-height:60vh}}
...
.qbox .opts{{display:grid;grid-template-columns:1fr;gap:10px;margin-top:12px;text-align:left}}.qbox .opts .btn{{justify-content:flex-start;min-height:48px;white-space:normal;line-height:1.4;padding:12px 14px;text-align:left;overflow-wrap:anywhere}}.qbox .opts .btn.correct{{border-color:var(--brand);background:color-mix(in srgb,var(--brand) 10%,transparent)}}.qbox .opts .btn.wrong{{border-color:var(--red);background:color-mix(in srgb,var(--red) 8%,transparent)}}
.qprog2{{height:6px;border-radius:99px;background:var(--bg2);margin:10px 0;overflow:hidden}}.qprog2 b{{display:block;height:100%;background:var(--brand);border-radius:99px;transition:width .3s}}
.qexplain{{margin-top:10px;padding:10px 12px;border-radius:10px;background:var(--bg2);border:1px solid var(--line-soft);font-size:13px;color:var(--mut);text-align:left}}
@media(max-width:600px){{.qbox{{padding:16px;margin:12px auto}}.qbox h3{{font-size:18px}}.qbox .opts{{gap:8px}}.qbox .opts .btn{{font-size:14px;min-height:44px}}}}
...
let _qi=0,_score=0,_streak=0,_qorder=[],_missed=[],_qtypes=[];function quiz(){{let n=Math.min(12, TERMS.length);let pool=[...TERMS];let hard=pool.filter((_,i)=>{{try{{return localStorage.getItem(KEY+TERMS[i][0].toLowerCase().replace(/[^a-z0-9]+/g,'-'))==='hard'}}catch(e){{return false}}}});if(hard.length>=4)pool=hard.concat(pool.filter(t=>!hard.includes(t)));_qorder=pool.sort(()=>Math.random()-.5).slice(0,n);_qtypes=_qorder.map(()=>Math.random()>.5?'td':'dt');_qi=0;_score=0;_streak=0;_missed=[];showQ();}};function showQ(){{const qb=$('#qbox');if(_qi>=_qorder.length){{const pct=Math.round(100*_score/_qorder.length);let rev=_missed.length?'<div style="text-align:left;margin-top:10px"><b>Review missed</b>'+_missed.map(m=>'<div style="margin:6px 0;padding:8px 10px;border:1px solid var(--line-soft);border-radius:8px"><b>'+m[0]+'</b> — '+m[1]+'<br><small style="color:var(--mut)">'+m[2]+' · <a href="#g-'+m[3]+'">card</a></small></div>').join('')+'</div>':'';qb.innerHTML='<div class="qbox"><div class="qprog2"><b style="width:100%"></b></div><h3>Done — '+_score+' / '+_qorder.length+' · '+pct+'%</h3><p class="sub">'+(_score===_qorder.length?'Perfect!':_score>_qorder.length/2?'Solid — review the misses':'Keep drilling weak cards')+'</p>'+rev+'<p><button class="btn" onclick="document.getElementById(\\'qbox\\').hidden=true">Back to cards</button> <button class="btn" onclick="quiz()">Again</button> '+(_missed.length?'<button class="btn" onclick="_qorder=_missed.slice();_qtypes=_qorder.map(()=>Math.random()>.5?\\'td\\':\\'dt\\');_qi=0;_score=0;_streak=0;_missed=[];showQ()">Retry missed</button>':'')+'</p></div>';qb.hidden=false;window.scrollTo({{top:qb.offsetTop-80}});return;}};const cur=_qorder[_qi];const typ=_qtypes[_qi];const isTD=typ==='td';const qtext=isTD?cur[0]:cur[1];const qhead=isTD?'What is '+cur[0]+'?' : 'Which term matches this definition?';let opts=[];if(isTD){{opts=[cur[1]];while(opts.length<4){{const r=TERMS[Math.floor(Math.random()*TERMS.length)][1];if(!opts.includes(r))opts.push(r);}}}}else{{opts=[cur[0]];while(opts.length<4){{const r=TERMS[Math.floor(Math.random()*TERMS.length)][0];if(!opts.includes(r))opts.push(r);}}}}opts.sort(()=>Math.random()-.5);const correct=isTD?cur[1]:cur[0];let html='<div class="qbox"><div class="qprog2"><b style="width:'+Math.round(100*_qi/_qorder.length)+'%"></b></div><p class="sub">Question '+(_qi+1)+' / '+_qorder.length+' · score '+_score+' · streak '+_streak+'</p><p class="sub" style="color:var(--brand)">'+(isTD?'Term → Definition':'Definition → Term')+'</p><h3>'+qtext+'</h3><div class="opts">'+opts.map(o=>'<button class="btn qo">'+o+'</button>').join('')+'</div><div id="qexplain" hidden></div><p><button class="btn" onclick="document.getElementById(\\'qbox\\').hidden=true">Exit quiz</button></p></div>';qb.innerHTML=html;qb.hidden=false;const exEl=qb.querySelector('#qexplain');qb.querySelectorAll('.qo').forEach(b=>{{b.onclick=()=>{{const ok=b.textContent===correct;const all=qb.querySelectorAll('.qo');all.forEach(x=>{{x.disabled=true;if(x.textContent===correct)x.classList.add('correct');else if(x===b&&!ok)x.classList.add('wrong');}});if(ok){{_score++;_streak++;}}else{{_streak=0;_missed.push(cur);}}const exp='<div class=\"qexplain\"><b>'+(ok?'✓ Correct':'✗ '+correct)+'</b> — '+cur[1]+'<br><small style=\"color:var(--mut)\">'+cur[2]+' · <a href=\"#g-'+cur[3]+'\">jump to card</a></small></div>';exEl.innerHTML=exp;exEl.hidden=false;setTimeout(()=>{{_qi++;showQ();}}, ok?900:1700);}};}});window.scrollTo({{top:qb.offsetTop-80}});}};$('#quizbtn').onclick=quiz;
const THS=['sepia','dark','light'],THI={{dark:'☀',light:'☾',sepia:'◐'}};
